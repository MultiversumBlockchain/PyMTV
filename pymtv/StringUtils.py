        ...
        if not quotedValue and input[pos] == quote:  # Start of a new quoted term
            ...
            if started and input[pos] not in [separater, linebreak]:
                ...
                if not line[-1]:
                    ...
                    soFar = 'start of line'
                raise ParserError('Invalid character after %s, term %s in line %s, expected %s or %s characters.' % (
                str(soFar), len(line), len(lines) + 1, repr(separater), repr(linebreak)))
...
if __name__ == '__main__':
    print
    smartSplit("'test1', NULL, h ", quote="'", swap={'NULL': None})
