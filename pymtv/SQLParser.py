...
from .StringUtils import stripBoth,stripEnd,stripStart
                ...
                if sql[pos:pos + 6].lower() == 'unique':
                    ...
                    if sql[pos:pos + 7].lower() == 'default':
                        ...
                        raise SQLSyntaxError(
                            "Expected DEFAULT or FOREIGN KEY or the end of the statement not %s for column %s in CREATE statement." % (
                            repr(sql[pos:pos + 11]), len(columns) + 1))
                    ...
                    if sql[pos] == ' ':
                        ...
                        for char in column['foreignKey']:
                            if char not in allowedCharacters:
                                raise SQLSyntaxError('Invalid character %s in FOREIGN KEY value %s for column %s' % (
                                repr(char), repr(column['foreignKey']), repr(column['name'])))
                        columns.append(column)
                        column = {
                            'name': None, 'type': None, 'unique': False, 'required': False, 'primaryKey': False,
                            'foreignKey': None, 'default': None,
                        ...
                        raise SQLSyntaxError(
                            "Non-whitespace character '%s' found after FOREIGN KEY definition of column %s" % (sql[pos], len(columns) + 1))
                else:
                    raise Bug('Programming Error')
        ...
        while 1:
            if position == -2:
                ...
                if not quote:
                    if where[pos] in ["'", ',', '\n']:
                        raise SQLSyntaxError(
                            "Invalid character '%s' found after operator in WHERE clause. Try removing the space after the operator or quoting the value." % (
                            where[pos]))
                    elif where[pos] == ' ':
                        if term.upper() == 'NULL':
                            terms.append('NULL')
        ...
        while 1:
            if position == 0:
                ...
                if not quote:
                    if update[pos] in ["'", '\n']:
                        raise SQLSyntaxError(
                            "Invalid character '%s' found after = in UPDATE statement. Try removing the space after the operator or quoting the value." % (
                            update[pos]))
                    elif update[pos] == ' ':
                        if term.upper() == 'NULL':
                            terms.append("NULL")
